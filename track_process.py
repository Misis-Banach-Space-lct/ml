...
frames = []
VID_STRIDE = 40 # Вот тут надо придумать как динамически определять частоту
# Пока что я делаю: < 1 минуты - 5, 2 - 5 минут - 20, 8 - 10 минут - 40

with torch.no_grad():
  results = model.track(source = '/content/drive/MyDrive/Копия 236.VC19.1.7 Набережная Табло 2023-09-16 20-50-00_000+0300 [10m0s].mp4',
                        save = True, stream = True, tracker = "bytetrack.yaml", vid_stride = VID_STRIDE, classes = [1, 2, 3],
                        device = 'cuda')
  for res in results:
    frames.append(res)
    ...
    for cadr in res:
      ...
      for obj in cadr.boxes.data:
        ...
        if objects[id].cnt < 20 or obj[-2] > objects[id].conf:
            ...
            coordinates = process_cadr(model_predictor.predict(source = crop_img, classes = [1, 2]), obj[-2])
